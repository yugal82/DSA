...
public:
    int recursion(int idx, int maxWeight, vector<int> &weight, vector<int> &value){
        ...
        int take = INT_MIN;
        if(weight[idx] <= maxWeight) take = value[idx] + recursion(idx-1, maxWeight-weight[idx], weight, value);
    ...
    int memoization(int idx, int maxWeight, vector<int> &weight, vector<int> &value, vector<vector<int>> &dp){
        ...
        int take = INT_MIN;
        if(weight[idx] <= maxWeight) take = value[idx] + memoization(idx-1, maxWeight-weight[idx], weight, value, dp);
